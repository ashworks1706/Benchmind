...
import { ArrowLeft, Download, Printer, Check, XCircle as XIcon, Clock, Github, Loader2 } from 'lucide-react';
...
export default function ReportDetailPage() {
  ...
  const { testCollections, agentData, loadFromLocalStorage, acceptFix, rejectFix, addStatusMessage } = useStore();
  
  const [isLoading, setIsLoading] = useState(true);
  const [expandedFix, setExpandedFix] = useState<string | null>(null);
  const [isPushing, setIsPushing] = useState(false);
  const [showPushConfirm, setShowPushConfirm] = useState(false);
  const [appliedToGithub, setAppliedToGithub] = useState(false);
  ...
  const handleAcceptFix = (fixId: string) => {
    acceptFix(fixId, sessionId);
  };

  const handleRejectFix = (fixId: string) => {
    rejectFix(fixId, sessionId);
  };
  ...
  const handlePushToGithub = async () => {
    ...
    try {
      ...
      if (result.result.success && result.result.pr_url) {
        ...
        setAppliedToGithub(true);
        setGithubPrUrl(result.result.pr_url);
  ...
  if (!collection || !session || !testReport) {
    return (
      <div className="min-h-screen pt-16 flex items-center justify-center">
        <div className="text-center space-y-4">
          <div className="text-xl font-semibold text-destructive">Report not found</div>
          <Button onClick={() => router.push(`/projects/${projectId}/reports`)}>
            Back to Reports
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen pt-16 bg-white dark:bg-gray-900">
      {/* Fixed Header */}
      <div className="sticky top-16 z-40 bg-white dark:bg-gray-900 border-b border-border">
      ...
      <div className="max-w-6xl mx-auto px-8 py-12">
        <div
          ref={reportRef}
          className="space-y-8 research-paper"
          style={{
                        ...
                        {fix.status === 'pending' && (
                          <div className="flex items-center gap-2 mt-3">
                            <Button
                              ...
                              onClick={() => handleAcceptFix(fix.id)}
                            >
